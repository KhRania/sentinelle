...
router.get('/api/robot/log', (req, res) =>
{
	RobotLink.api({url: '/log', encoding: null}, (err, response) =>
	{
		res.send(err ? '' : response.body);
...
router.get('/api/robot/area', (req, res) =>
{
	RobotLink.api({url: '/area', encoding: null}, (err, response) =>
	{
		res.send(err ? '' : response.body);
...
router.get('/api/robot/configure', (req, res) =>
{
	RobotLink.api({url: '/configure', encoding: null}, (err, response) =>
	{
		res.send(err ? '' : response.body);
...
router.get('/api/robot/patrol', (req, res) =>
{
	RobotLink.api({url: '/patrol', encoding: null}, (err, response) =>
	{
		res.send(err ? '' : response.body);
...
router.get('/api/robot/configure', function(req,res) {
	log('fetching jobs');
	jobDetails.find(function(err,jobs) {
	  if(err) {
	log("hhvhvhvhvvhv",err);
	  }
	  else {
		res.json(jobs);
	  }
...
/*router.get('/api/robot/goto', (req, res) =>
{
	RobotLink.api({url: '/goto?', encoding: null}, (err, response) =>
	{
		if (err || response.statusCode !== 200)
		{
			res.send(new RestResponse(false));
		...
		else
		{
			...
			{
				res.send(new RestResponse(true).set('x=', x).set('&y=', y));
			}
			else
			{
				res.send(new RestResponse(false));
	...
	router.post('/api/robot/goto', (req, res) =>
	{
		const {x, y,theta} = req.body;
		if( x !== undefined && y !== undefined )
		{ 
			RobotLink.api({url: '/goto?x=' + x + '&y=' + y} );
			res.send(new RestResponse(true));
