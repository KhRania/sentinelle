	...
	render()
	{
		...
		return (
			<div id='login-container' className='offset-page-content'>
				<Segment>
					<h1 id='authentifcation'>{i18n.get('authentication')}</h1>
					{errorMessage}
					<Form onSubmit={this.onSubmit}>
						<Form.Input id={this.loginInputId} name='login'  placeholder={i18n.get('login')} text={this.state.login} onChange={this.onValueChanged} />
						...
						<Button primary type='submit' floated={'right'} id='botton'>{i18n.get('login')}</Button>
					</Form>
				</Segment>
			</div>
		);
