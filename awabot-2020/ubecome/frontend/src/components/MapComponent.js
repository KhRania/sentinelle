    ...
    ajax("GET", "robot/map/size", null, response => {
      if (response.ok) {
       ...
       this.subscribeEvent("robot-goal", Coords => {
          const { goal_x, goal_y, goal_theta } = this.processRobotCoordGoal(Coords) ;

          this.setState({ goal_x, goal_y, goal_theta});
...
//console.log(test)   
arr.map(dataitems=>{ console.log("items",dataitems)
...
//console.log("gfhgfhdghgh",arr)
  ...
  render() {
    const { x, y, theta, mapWidth,goal_x,goal_y,goal_theta, mapHeight} = this.state;
...
//console.log(this.state.data)
  ...
  <image x='300' y='300'theta= {goal_theta }
                  width="30" height="30"
     href="cercle.gif" />
     </svg>
 ...
 {
 {points} = this.state.points.map((items,i) => 

(
<circle key={i} cx={items.x} cy={mapHeight-(items.y)} r="2" stroke="#FECFC5" >
 ...
 <path key={i}
                  d={item}     stroke-opacity="0.8"  fill="#F6A492 " fillOpacity="0.02" stroke="#FECFC5"
                  />
) }
